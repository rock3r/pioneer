...
| Review authentication | `src/pi-auth-broker.ts`, `src/pi-auth-runtime.ts`, `src/pi-auth-client.ts`, `src/pi-auth-worker.ts` | Broker OAuth refreshes through a separate sandboxed, model-free worker; persist only provider-scoped results under Pi's source credential lock |
...
| Review Git inspection | `src/review/git-inspect.ts` | Parse explicit `--git` targets, infer conservative Git-target prompts, and collect bounded read-only Git context in the controller |
...
3. When the review names explicit `--git` targets or the prompt is a Git-target request, collect bounded read-only Git context in the controller with a canonical Git executable, argument arrays, and an allowlist that disables hooks, pager, editor, external-diff, credential helpers, LFS filters, and unsafe protocols. `--source` must be the repository root. Validated refs and command output are treated as untrusted and injected into the review prompt. GitHub pull-request targets fail closed and ask for an explicit `--git` value. Collected targets are part of immutable resume scope.
4. Resolve Pi from the controller's selected `PATH`, run `pi --version`, enforce the supported range, and prepare private configuration and enabled-extension snapshots, then run model discovery inside the native sandbox. On Windows, PATH lookup follows `PATHEXT`; an npm `pi.cmd` is read only as a bounded generated shim, its target must match the installed `@earendil-works/pi-coding-agent` package's declared `pi` entry point, and Pioneer launches that target through its own absolute Node executable with discrete argv rather than a command shell. The same resolved command is reused for review startup. Reject an invalid `models.json` rather than using Pi's partial catalog. Newer-than-tested Pi versions continue with a warning; older or malformed versions fail before model discovery. If Pi reports no models, use access-only filesystem probes to distinguish missing configuration from an outer agent sandbox that hides Pi's agent directory. Readiness uses an allowlisted runtime environment and does not inherit provider secrets or outer-agent control state. Controller-side runtime preparation also keeps `PI_CODING_AGENT_SESSION_DIR` so it can exclude private session storage; that preparation environment never reaches the sandboxed actor.
5. Resolve a requested qualified model exactly, or an unqualified model only when it is unique.
6. Copy a selective snapshot of `PI_CODING_AGENT_DIR` (default `~/.pi/agent`) into a private run directory. The default root allowlist is `auth.json`, `models.json`, `models-store.json`, `settings.json`, and `AGENTS.md`; review snapshots also include `skills/`, while eval snapshots deliberately do not. Traversal of `skills/` skips `node_modules/`, `.git/`, transient directories, and log files; root-level `npm/`, `git/`, and unknown paths are never traversed by the allowlist, while valid skill directories such as `skills/git/` remain available. Review callers may add repeated exact relative paths with `--pi-home-include`; evals have no equivalent opt-in. Sessions, logs, `.npm`, `.cache`, `tmp`, `.tmp`, `temp`, and log files are hard exclusions, with case-insensitive matching on macOS and Windows. Below a `node_modules` segment, the directory names `sessions`, `logs`, `tmp`, `.tmp`, and `temp` are dependency source and are not excluded. A session directory configured through `PI_CODING_AGENT_SESSION_DIR` or the `sessionDir` setting is excluded wherever it lies inside the Pi home. Selected content alone consumes the 500,000-entry and 1 GiB backstops.
7. Build an ephemeral `pi --mode rpc` command with offline startup, a private native session directory by default (or `--no-session` for the explicit opt-out), no approval, explicit staged user extensions with ambient discovery disabled, no prompt-template discovery, no theme discovery, and an allowlist of Pi's built-in inspection tools. Token loading acquires the retained archive lease before reading or validating its manifest, scope, attempts, or session tree; validation failure releases that exact lease, while success transfers it across readiness, attempt copy, and execution so concurrent pruning cannot remove the recovery point. The allowlist excludes `write`, `edit`, `bash`, `grep`, and `find` outside Linux; all platforms retain `read` and `ls` so source-only reviews can discover files without process creation. Linux additionally permits `bash` inside Bubblewrap. Git-target inspection is collected by the controller on every platform, so macOS and Windows no longer reject those reviews.
8. Reject `--network none` for reviews before launch: the supported Pi actor needs proxy-mediated egress to its configured model provider. Start an authenticated loopback proxy for `public` or `full` networking.
...
12. Publish Pi's verified final Markdown report through the controller-owned reservation inode and return it. Never overwrite or delete a target that no longer matches the reservation, require the matching randomly named sibling link and live OS process-start identity when classifying a report as actively reserved, and hold a separate random publication lease while the reservation inode is being changed so concurrent retention cannot unlink it. Restore the reservation marker after a failed write; after sync and final target-identity validation, treat handle close and immediate sibling removal as best-effort cleanup that cannot roll the durable report back. Reclaim abandoned reservation and publication siblings and inactive post-publication hard links during later default-report retention, while independently protecting live sidecar owners before target-link creation and granting incomplete markers a one-minute setup grace. On strict success delete the native archive along with the proxy, bridge, Pi snapshot, and scratch directory; on failure remove an unpublished reservation. After a proven non-success, retain only a bounded regular native session tree containing exactly one native `.jsonl` session and emit an opaque resume token; cap a committed attempt at half the aggregate archive budget so the next crash-safe copy is always possible. Before a resume copy, remove crash-left staging attempts while holding the archive lease, and reject explicit or derived default output paths anywhere inside the private resume store. Persistence failures retain the archive for resume. If Pi rejects a copied native session, atomically restore the manifest to the validated prior attempt instead of retaining the rejected copy. Post-retention pruning is best effort, acquires the candidate's resume lease before deletion, and cannot roll back, race a claimed recovery point, or invalidate an already committed recovery attempt. Work-log creation or flushing failures stop the review because an unobservable run is not allowed to continue. Runs with resumption disabled never create or prune resume storage.

See [REVIEW-TRANSPORT.md](REVIEW-TRANSPORT.md) for the RPC contract and [SECURITY.md](SECURITY.md) for the trust boundaries.

## Review OAuth lifecycle

On native platforms, review preparation creates an authenticated refresh broker when the private snapshot contains OAuth credentials. The Pi adapter redirects credential refresh operations to that broker. The broker accepts only an existing provider name; the controller reads its current source credential under Pi's shared lock and runs the same extension snapshot in a separate model-free sandbox. That worker has no source or review-scratch grant and uses the review's network policy. Its validated provider result is persisted before cleanup and returned to the review's private credential store. Concurrent reviews and ordinary Pi therefore reuse rotations instead of invalidating each other's copied refresh tokens. The review actor never controls the data persisted to the source credential file. See [security details](SECURITY.md#oauth-refresh-in-native-reviews-and-evals).

## Eval lifecycle

Eval preparation validates `skill_name` as one portable path component within the cross-platform filename byte limit, rejects fixture paths and contents that leak the expected finding with `[EVAL_FIXTURE_LEAK]`, canonicalizes the proposed output parent, and revalidates the created destination before populating it, then creates controller-only metadata plus independent `baseline` and `with-skill` actor directories. Only the with-skill arm receives a sanitized skill copy. Eval execution rejects broad or protected-system writable run directories, caller runtime-read grants that overlap the writable run tree, Pi homes that overlap any actor grant, and Pi package roots that overlap that tree before constructing grants. Controller-derived executable paths already covered by the writable run grant are omitted from the read-only grant set. Controller launch/probe files and the selective Pi snapshot live in a canonical private temporary tree outside the persistent actor run. That tree is rooted in `/tmp` on macOS and Linux or `os.tmpdir()` on Windows by default; the short Unix base keeps the Linux proxy bridge socket inside `sun_path`, and the bridge root is allocated from the same base so the two cannot drift. A caller may supply its own base through `controllerScratchBase`, on an eval run or a review request alike, which is canonicalized and rejected when it is missing, not a directory, a broad or protected system location, or too long to host the bridge socket. The actor's write grants follow that base, so an override is held to the same standard as a writable run directory. The actor receives the selected Pi configuration in an ephemeral isolated `agentDir` that is writable so current Pi can create credential lock directories such as `auth.json.lock` and `settings.json.lock`, plus separate writable home/tmp scratch; the source Pi home is never mounted. When the actor is the trusted official Pi package, Pioneer also stages that package's enabled user extensions into a read-only directory, passes those paths with `--extension`, and keeps `--no-extensions` so Pi does not discover any other extensions. The same adapter removes extension tools. Provider and authentication hooks still run, which is what makes an extension-registered model resolvable. A Pi command that passes `--no-extensions` or `-ne` stays on built-in providers and still brokers OAuth when the snapshot contains OAuth credentials and the package can host the auth adapter. An explicit `--extension` or `-e` on that command still loads through the tool-stripping adapter. Eval snapshots still have no `--pi-home-include`. An outer interruption scope covers validation, readiness, snapshotting, probes, launch, and cleanup, so all temporary material is removed on success, failure, or signal. Every eval run also writes a controller-owned JSONL work log (default or `--work-log`) and prints `[PIONEER_EVAL_WORK_LOG] ABSOLUTE_PATH` after opening it. Before every actor launch, the runner proves that the sandbox cannot read or modify an outside sentinel, inherit a host-only secret, or connect directly to a host loopback listener.
...
The capture controller starts the native sandbox in a distinct process group on macOS and Linux, waits for close after stdio closure, and forwards timeout or interruption signals to the group. A bounded pipe-close grace period detects descendants that retain inherited pipes; those runs fail with `[EVAL_PROCESS_CONTAINMENT_FAILED]`. On Linux the actor's PID namespace destroys such a descendant with the run, so containment is structural there and that diagnostic is not reachable. Timeout, interruption, spawn, and output-limit failures remain nonzero and preserve bounded partial stdout/stderr.
