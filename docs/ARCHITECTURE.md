...
8. Reject `--network none` for reviews before launch: the supported Pi actor has no extension-discovery escape hatch and needs proxy-mediated egress to its configured model provider. Start an authenticated loopback proxy for `public` or `full` networking.
