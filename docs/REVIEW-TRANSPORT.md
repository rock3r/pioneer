...
Pioneer also converts each Pi RPC record into a sanitized work-log event as it arrives. Event type/subtype, byte counts, tool name/call ID, retry counters/delays, compaction reason, settlement state, and redacted bounded diagnostic text are retained; payload content is not. Stderr contributes byte-count activity records, while its bounded text remains only in terminal transport diagnostics. A five-second heartbeat records the last Pi event, idle duration, RPC/stderr byte counts, and child PID. Missing heartbeats distinguish a stalled controller/event loop from a live controller waiting on silent Pi activity.
