...
Native resume archives are private per-user application-data directories with mode `0700` directories and `0600` manifests on POSIX; Windows relies on the per-user application-data ACL. XDG and Windows application-data overrides must be absolute. Pioneer canonicalizes the archive root and rejects it when it overlaps the source or any actor-visible read/write grant, so neither the archive parent nor the active session grant can enter an actor-visible tree. The token is an identifier, not a filesystem capability. The manifest stores only schema, exact Pi version, scope metadata, and prompt digest—not prompts, reports, RPC streams, credentials, or session content; manifest reads are bounded at 1 MiB. Before retention Pioneer waits for process-tree close, rejects symlinks and special files, and enforces 64 MiB/10,000-file bounds. Resume validates UUID containment, manifest integrity, source/grant scope, exact Pi version, and a new Windows acknowledgement; attempt promotion stages and validates a copy while preserving symlink entries, atomically renames it, atomically replaces the manifest, and loads only the manifest's committed attempt, so a controller crash cannot strand the prior valid attempt. Lease acquisition and release verify the exact private lease contents, probes fail closed on an `EPERM` PID check, and stale staging/manifest temporary files are reclaimed after retention. Failed containment never yields a token.
