...
Pioneer checks only the fixed `@rock3r/pioneer` npm package name and public npm registry, with fixed npm arguments, bounded output, and a five-second timeout. npm runs from the operator's home directory with a constrained environment, so repository-local npm configuration and ambient npm tokens cannot affect either the check or install. Normal checks are best-effort: they run in parallel with the requested command, do not affect its result, and cache both successful and failed check times for 24 hours. The cache contains only a timestamp and the public package version.
