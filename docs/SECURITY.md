...
- Git-target reviews never grant Pi a shell on macOS or Windows. The controller resolves Git canonically, runs only `status`, `diff`, `show`, and `rev-parse` with disabled hooks, pager, editor, external-diff, credential helpers, LFS filters, and `file`/`ext` protocols, reads attributes from the empty tree so repository clean/smudge filters cannot execute, validates refs and object names, and bounds each command to 256 KiB and the assembled context to 1 MiB. Repository output is injected into the prompt as untrusted text. `--source` must be the repository root. Unrecognized Git-target prompts fail closed instead of substituting the working tree.
