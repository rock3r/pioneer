...
- an optional `--report` target is controller-owned, create-only, and never granted to the actor; it must be absolute, absent, and outside every actor-visible grant; Pioneer exclusively reserves it with a private random ownership marker plus a randomly named controller-owned sibling reservation link before announcing the path, so untrusted report text cannot impersonate an active reservation and an orphaned link cannot block reuse of an absent target, and atomically publishes only over that still-owned reservation;
