...
The writable `--run-dir` and every read-only `--runtime-read` grant must be narrowly scoped and must not overlap. Writable protected-system roots and their descendants, plus broad filesystem, sensitive-configuration, home, temporary, and variable-data roots and their canonical platform aliases, are rejected. Narrow read-only system runtimes required by an actor and disposable temporary descendants remain supported.
