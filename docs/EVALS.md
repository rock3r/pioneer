...
The prepared output must not already exist and its canonical parent must place it outside the source skill. Pioneer revalidates the created destination before populating descendants. `skill_name` must be one portable path component within the 255-byte filename limit; absolute, empty, dot, separator-containing, Windows-reserved, invalid-character, and trailing-dot/space values are rejected before any output directory is created. Symlinks in the source skill are rejected. Staged fixture paths and contents that leak the expected finding fail closed with `[EVAL_FIXTURE_LEAK]` before output is created; `--allow-fixture-name GLOB` waives a genuine filename collision only. Each eval gets independent baseline and with-skill directories; only the latter receives a sanitized skill copy. Expectations and answer keys never enter actor-visible storage. Paths are not similarly hidden: the actor sees every staged filename.
