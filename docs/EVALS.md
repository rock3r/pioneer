...
When the actor command is Pi, or the declared `pi` entry point of the trusted official package, the runner adds `--offline`, `--no-session`, `--no-approve`, `--no-extensions`, `--no-prompt-templates`, `--no-themes`, and `--no-skills`, plus `PI_OFFLINE=1` and `PI_TELEMETRY=0`. For that official package it also adds one `--extension` path per staged enabled user extension and launches Pi through the tool-stripping adapter. `--no-extensions` or `-ne` on the Pi command skips user-extension staging and keeps built-in providers only. When the snapshot contains OAuth credentials and the package can host the auth adapter, that opt-out still starts the provider-scoped broker and launches through the auth adapter. An explicit `--extension` on that command also launches through the tool-stripping adapter and does not stage the enabled user set. A trusted package that cannot host the adapter rejects the explicit extension.
