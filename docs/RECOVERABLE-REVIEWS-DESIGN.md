...
- Retain unsuccessful archives for seven days; retain at most ten, with 64 MiB and 10,000-entry caps per archive measured only after proven process-tree termination. Reject and prune lifecycle timestamps more than five minutes in the future so clock correction or corrupted metadata cannot extend retention indefinitely. A committed attempt may consume at most half of each archive cap, reserving enough capacity for the crash-safe copy required by the next resume.
- Validate a prospective resume root against all actor-visible grants and the Pi-home snapshot source before creating or changing permissions on it. Prune only inactive expired archives before creation and after terminal cleanup; never prune an active lease. Token loading acquires the archive lease before validating any retained contents, releases it on validation failure, and transfers it to the resume lifecycle on success. Leases bind the PID to its OS process-start identity so PID reuse does not preserve or take over an archive. Lease publication and restoration use create-only hard links, so a stale-takeover race cannot overwrite a newly published contender. A losing restoration preserves the displaced inode; each publisher checks for a live displaced owner and backs off before returning acquisition. Concurrent pruning tolerates candidates removed after directory enumeration.
- A controller crash before the archive reaches a proven retained or report-delivery-failed state leaves it `active`; resume rejects that archive because the prior actor cannot be proven stopped, and normal expiry/count pruning eventually removes it.
- Deletion is ordinary filesystem deletion, not cryptographic erasure.
- Session content, token, prompt, thinking, tool inputs/results, credentials, proxy values, and raw provider diagnostics never enter Pioneer-generated work-log fields or errors. Every present immutable-scope field is type-checked at load time rather than filtered or normalized. The final report remains Pi's independently generated Markdown and is stored privately.
