...
describe("Pi version policy", () => {
  ...
  it("warns without failing for versions newer than the tested maximum", () => {
    expect(validatePiVersion("0.82.1")).toEqual({
      version: "0.82.1",
      warning:
        "[PI_VERSION_UNTESTED] Pi 0.82.1 is newer than the newest version tested with this Pioneer release (0.82.0). Continuing because the CLI contract may still be compatible.",
    });
  });
  ...
  it("rejects version output that is not semantic", () => {
    expect(validatePiVersion("development")).toEqual({
      version: "development",
      error:
        "[PI_VERSION_UNRECOGNIZED] Pi returned an unrecognized version: development. Install a released Pi version between 0.80.6 and 0.82.0, or newer with a compatibility warning.",
    });
  });
});
