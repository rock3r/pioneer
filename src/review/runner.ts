...
export function requiresGitInspection(prompt: string): boolean {
  return /\b(?:review|inspect|compare)\b[^.]*\b(?:staged|unstaged|working[-\s]tree|commit\s+`?[0-9a-f]{6,64}`?|this\s+branch|branch\s+(?:against|with|compared)|merge\s+base|(?:the\s+)?diff(?:\s*(?:$|[.?!])|\s+(?:against|between|of|from)\b))/i.test(
    prompt,
  );
}
