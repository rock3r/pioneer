...
export function requiresGitInspection(prompt: string): boolean {
  return /\b(?:review|inspect|compare)\b[^.]*\b(?:staged|unstaged|working[-\s]tree|current\s+changes|commit\s+`?(?=[0-9a-f]{6,64}`?\b)(?=[0-9a-f`]*\d)[0-9a-f]{6,64}`?|this\s+branch|branch\s+(?:against|with|compared)|merge\s+base|(?:the\s+)?diff(?:\s*(?:$|[.?!])|\s+(?:against|between|of|from)\b)|against\s+origin\/|(?:changes|commit|branch|diff)\b[^.]*\bagainst\s+(?:HEAD|main|master)\b|(?:main|master|HEAD|origin\/[0-9a-z._/-]+)\.{2,3}[0-9a-z._/-]+|[0-9a-z._/-]+\.{2,3}(?:main|master|HEAD|origin\/[0-9a-z._/-]+))/i.test(
    prompt,
  );
}
