...
describe("Pi package runtime hosting", () => {
  ...
  it("honors XDG_CONFIG_HOME for credential directories", () => {
    ...
    expect(
      isXdgConfigCredentialPath("/srv/xdg/age/keys.txt", "linux", {
        XDG_CONFIG_HOME: "/srv/xdg",
      }),
    ).toBe(true);
    expect(
    ...
    expect(
      isXdgConfigCredentialPath("/srv/xdg/hcloud/cli.toml", "linux", {
        XDG_CONFIG_HOME: "/srv/xdg",
      }),
    ).toBe(true);
    expect(
  ...
  it("folds credential directory names on case-insensitive platforms", () => {
    ...
    expect(
      isSensitiveCredentialPath(`${home}${path.sep}.config${path.sep}hcloud${path.sep}cli.toml`),
    ).toBe(true);
    expect(
      isSensitiveCredentialPath(
