...
- Native sandbox tests need `bwrap` (Bubblewrap) and `/sbin/apparmor_parser` (apparmor), which are baked into the base snapshot. `.github/scripts/install-linux-sandbox-prereqs.sh` documents the exact packages. Without `bwrap`, `npm run sandbox:smoke` does **not** skip: `scripts/native-sandbox-smoke.mjs` checks `resolveLinuxBwrapPath()` and throws `Linux sandbox smoke requires Bubblewrap`, so the command exits nonzero. End-to-end cases that launch an actor skip when sandbox readiness fails (`describe.skipIf(!sandboxReady)`); `eval prepare` cases still run. A skipped e2e file is not a passing smoke command.
