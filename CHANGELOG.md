...
- Certify Pi `0.87.1` as the newest tested compatibility endpoint after reviewing the [0.87.1](https://github.com/earendil-works/pi/releases/tag/v0.87.1) release (frontier models, the Grok 4.7 xAI default, compaction summaries, `--mode` validation, image-only messages, and the Anthropic OAuth version string). Required CLI options and thinking levels are unchanged; Pioneer still passes `--mode rpc`. The minimum stays `0.80.6`.
...
- Preserve OAuth token rotations during native reviews through a separate sandboxed authentication worker and Pi's shared credential lock. Concurrent reviews reuse fresh credentials; review-agent changes to private credential files are never copied back.
- Preserve replacement report files and sidecars when Windows reuses a deleted file's identity during publication cleanup.
