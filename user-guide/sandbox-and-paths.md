...
Both modes cap selected content at 500,000 entries and 1 GiB; skipped content does not consume those budgets. Opting into large or machine-specific directories increases storage, runtime, and portability risk. Optional extension discovery remains disabled for both review and eval actors. Eval snapshots use an external private temporary tree: selected Pi configuration is read-only, writable home/tmp scratch is separate, and everything is removed when the run ends.
